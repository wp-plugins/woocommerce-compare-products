...
class WOO_Compare_Class{	
	...
	function woo_compare_manager(){
		...
		if(isset($_REQUEST['bt_save_settings'])){
			...
			update_option('woo_comparable_settings', $comparable_settings);
			$comparable_setting_msg = '<div class="updated" id="comparable_settings_msg"><p>'.__('Save Compare Settings successfully','woo_cp').'.</p></div>';
		}elseif(isset($_REQUEST['bt_reset_settings'])){
			WOO_Compare_Class::woocp_set_setting_default(true);
			$comparable_setting_msg = '<div class="updated" id="comparable_settings_msg"><p>'.__('Reset Compare Settings successfully','woo_cp').'.</p></div>';
		}
		...
		if(isset($_REQUEST['bt_save_field'])){
			if(isset($_REQUEST['field_id']) && $_REQUEST['field_id'] > 0){
				if(trim($_REQUEST['field_key']) == '' || WOO_Compare_Data::check_field_key_for_update($_REQUEST['field_id'], $_REQUEST['field_key'])){
					...
					$result_msg = '<div class="updated" id="result_msg"><p>'.__('Edit Compare Feature successfully','woo_cp').'.</p></div>';
				}else{
					$result_msg = '<div class="error" id="result_msg"><p>'.__('This Feature Key is existed, please enter other Feature Key','woo_cp').'.</p></div>';
				}
			}else{
				if(trim($_REQUEST['field_key']) == '' || WOO_Compare_Data::check_field_key($_REQUEST['field_key'])){
					...
					if($result > 0){
						$result_msg = '<div class="updated" id="result_msg"><p>'.__('Create Compare Feature successfully','woo_cp').'.</p></div>';
					}else{
						$result_msg = '<div class="error" id="result_msg"><p>'.__('Create Compare Feature error','woo_cp').'.</p></div>';
					}
				}else{
					$result_msg = '<div class="error" id="result_msg"><p>'.__('This Feature Key is existed, please enter other Feature Key','woo_cp').'.</p></div>';
				}
		...
		if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'delete'){
			...
			$result_msg = '<div class="updated" id="result_msg"><p>'.__('Delete Compare Feature successfully','woo_cp').'.</p></div>';
		}
		?>
        <style>
...
<div class="wrap">        
	...
	<div style="width:70%; float:left;">
        <div id="icon-themes" class="icon32">&nbsp;</div><h2><?php if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'edit'){ _e('WooCommerce Compare Products Lite','woo_cp'); }else{ _e('WooCommerce Compare Products Lite','woo_cp'); } ?></h2>
        ...
        <ul>
			<li>
				...
				<div class="woocp_description_text" style="display:none">
        			...
        			<p><?php _e('STEP 1. Scroll down this page and you will See the Compare Products Setup section. It is the first thing you should do because in the Lite Version the feature is auto added to every product on your site so you should set it to look the way you want it to. The set up section is at the bottom of the page because after you have done the set up you will rarely use it again.','woo_cp'); ?></p>
                    <p><?php _e('STEP 2. Create Compare Product Features. Compare Products Lite has only 1 Master Compare Product Category and all you have to do is add the features. In the PRO version you Create Compare Product Categories and then add the Features to the Category. This is another reason that the PRO version is more suitable to larger sites that have a variety of products which of course have different feature sets.','woo_cp'); ?></p>
            ...
            <li>
				...
				<div class="woocp_description_text" style="display:none">
                    ...
                    <li><?php _e('5. Edit Features and Fields on the product page and update your post to save the changes.','woo_cp'); ?></li>
                ...
                <tbody>
                	<tr>
                    	<td>
							...
							<div style="margin-left:200px;"><?php _e('Please do not enter space character.','woo_cp'); ?></div>
                                        ...
                                        <li>
                                            ...
                                            <div class="woocp_description_text" style="display:none">
                                                <p>
                                                    ...
                                                    <?php _e("Mutli Select - Like Drop Down but allows you to Mutiple select Options",'woo_cp'); ?>
                                                </p>
                                                <p>&nbsp;</p>
                                            </div>
                                        </li>
        ...
        <div style="clear:both; margin-bottom:20px;"></div>
        <?php // } ?>     
        <div id="icon-themes" class="icon32"><br></div><h2><?php _e('WooCommerce Compare Products Settings','woo_cp'); ?></h2>
                ...
                <tbody>
                	...
                	<tr>
                    	<td>
                            ...
                            <div style="margin-left:200px;"><?php _e('To add a header image to your Compare Fly-Out screen put the full URL of the image you want to use. Use file formats .jpg .png. jpeg Your image can be any size. If it is not as wide as the Fly-Out Container width you set below it will sit in the centre at the top of the Fly-Out screen. If it is wider the bottom scroll bar will come into play.','woo_cp'); ?></div>
                            <div style="clear:both; height:20px;"></div>
                            <div style="width:200px; float:left"><label for="popup_width"><?php _e('Comapre Fly-Out Width','woo_cp'); ?></label></div> <input type="text" name="popup_width" id="popup_width" value="<?php echo $comparable_settings['popup_width'] ?>" style="width:100px" />px
                            <div style="clear:both; height:20px;"></div>
...
','woo_cp'); ?></div>
                    ...
                    <tr>
                    	<td>
                            ...
                            <div style="margin-left:200px;"><?php _e("If you select LINK - the hyperlinked text auto shows in the same font and colour as set by your themes style sheets. Unfortunately is not possible to auto know the Style and Colour of your themes BUTTONS as many themes have many different buttons. If you have a little coding knowledge you can easily manually change the style and Colour of the Compare button used as default by us. All objects in the plugin have a class so you can style them. The Compare button on the product pages class name is 'bt_compare_this' and class name and the Compare button in the sidebar widget class name is 'compare_button_go'. Find the button style in you style sheets that you want to use and apply it to the plugin classes to change the buttons to your themes style.
",'woo_cp'); ?></div>
                            <div style="clear:both; height:20px;"></div>
                    ...
                    <tr>
                    	<td>
                            ...
                            <div style="margin-left:200px;">
                            <?php _e('If your theme does not use the WooCommerce Product Page Nav Bar You can use this function','woo_cp'); ?> <code>&lt;?php if(function_exists('woo_show_compare_fields')) echo woo_show_compare_fields(); ?&gt;</code> <?php _e('to put into your theme code where you want to show Compare Featured fields','woo_cp'); ?>
                            </div>
                            <div style="clear:both; height:20px;"></div>
                    	</td>
                    </tr>
    ...
    <div style="float:right; width:25%; margin-left:5%; margin-top:60px;">
